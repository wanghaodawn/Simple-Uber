/*
 * Project 4.3 Bonus - Implement a uber-like service by Kafka and Samza
 * Hao Wang - haow2
 */
...
public class DriverMatchTask implements StreamTask, InitableTask, WindowableTask {
  ...
  private KeyValueStore<String, Queue<Integer>> driver_hist;
  private final int MAX_DIST = 500;
  private final double SPF_THRESHOLD = 3.6;
  ...
  public void init(Config config, TaskContext context) throws Exception {
    ...
    driver_hist = (KeyValueStore<String, Queue<Integer>>) context.getStore("driver-loc"); 
  ...
  private void process_request(int block_id, int client_id, int latitude, int longitude, String gender_preference, MessageCollector collector) {
    ...
    // Handle the situation when map.size() == 0
    if (map == null) {
      map_size = 0;
      return;
    } else {
    ...
    // Compute the Surge Pricing Factor (SPF)
    Queue<Integer> queue = driver_hist.get(""+block_id);
    if (queue == null) {
    ...
    if (queue.size() < 4) {
      queue.add(map_size);
      driver_hist.put(""+block_id, queue);
    } else {
      // For later requests
      // Maintain the size of the queue as QUEUE_SIZE
      ...
      // Traverse the queue to compute the mean value
      double mean_val = 0.0;
      for (int i = 0; i < QUEUE_SIZE; i++) {
        System.out.println("Client: " + client_id + "\tQueue: " + queue.peek());
        mean_val += queue.poll();
      }
      assert queue.size() == 0;
      mean_val /= QUEUE_SIZE * 1.0;
      System.out.println("Client: " + client_id + "\tmean_val: " + mean_val);

      // Compute the SPF
      if (mean_val >= SPF_THRESHOLD) {
        spf = 1.0;
      } else {
        spf = 1.0 + (4.0 * (3.6 - mean_val)/(1.8 - 1.0));
      }
    }

    System.out.println("Client: " + client_id + "\tSPF: " + spf);

    // Remove the driver from the list
    if (map.containsKey(best_driver)) {
      map.remove(""+best_driver);
      driver_list.put(""+block_id, map);
    }

    new_match.put("priceFactor", spf);
